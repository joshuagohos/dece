% Script to compute degree centrality .nii images.
% Created by Josh Goh 14 July 2022.
tic
clear all

% Set variables
gpu  = 1; % Use GPU if available, yes: 1, no otherwise.
...
if ~isempty(gpuDevice()) & gpu,
  disp('GPU detected and will be used.')
  Y = gpuArray(Y);
else
  disp('No GPU detected. Standard processors used.')
end
V = spm_vol(epifn);
...
for b_col = 1:nbatch
  ...
  if ~isempty(gpuDevice()) & gpu,
    R = zeros(size(Y,2),length(batch(b_col).vox_range));
    for b_row = 1:nbatch
      R(batch(b_row).vox_range,:) = gather(corr(Y(:,batch(b_row).vox_range),Y(:,batch(b_col).vox_range)));
    end
  else
